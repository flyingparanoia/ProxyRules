...
* **商业订阅1 (大型多节点中转) 原生配置性能灾难与发热诊断**：
  * 通过实测日志诊断出商业“订阅1”默认配置中 3,523 条 Classical 线性扫描规则引发的 CPU 高耗、26 个测速 Worker 的后台并发空转，以及末尾缺失 `no-resolve` 引发的 DoH 级联解析风暴，从而确立了本项目“前缀树（Trie）编译 + Fake-IP + no-resolve 强解耦”的高性能架构。
...
结合自研精细配置与“订阅1 / 订阅2”多源权威规则深度提取与合并，涵盖：
...
## 移动端性能避坑指南：商业“订阅1”（3,500+ Classical规则）在 iOS 端的性能灾难分析

在 iOS 环境下使用 Stash / Clash 等代理客户端时，许多用户常遭遇**机身严重发烫、掉电如崩盘、甚至代理频繁后台被系统杀死（闪退断网）**的问题。经对知名大型中转机场（以商业“订阅1”官方导出的 Stash/Clash 完整单文件配置及实机内核日志 `2026-09-26-055305.log`，测试时长不足 2 分钟）进行深度逆向与追踪分析，揭示了传统臃肿配置在移动端性能极其糟糕的五大底层架构缺陷。
...
* **机制缺陷**：
  ...
  商业“订阅1”官方配置将多达 **3,523 条规则**全部以明文形式平铺在配置文件的 `rules:` 根节点下。此类古典规则无法被编译为前缀树（Trie Tree）或基数树（Radix Tree），每一个数据包都必须经历从第 1 条逐行扫描到第 3,523 条的低效线性比对。
* **实测日志数据**：
...
* **机制缺陷**：
  ...
  商业“订阅1”官方配置内定义了 172 个节点，并在多达 31 个策略组中**全量重复平铺**这 172 个节点（产生 5,300+ 个节点引用对象），同时承载 3,500+ 个规则数据结构。
* **实测日志数据**：
  ```log
...
### 2. 架构对比：商业“订阅1”官方配置 vs 本项目优化架构

| 评估维度 | 商业“订阅1”官方默认配置 (3523条规则) | 本项目优化架构 (`ProxyRules`) |
| :--- | :--- | :--- |
...
**强烈建议切勿在 iPhone 等移动设备上直接运行服务商导出的原生臃肿单文件配置**。最佳的使用姿势为：

1. **提取纯节点信息**：
   仅提取服务商订阅中的节点链接，或将订阅链接作为 `proxy-providers` 引入；
2. **挂载本项目高性能规则集**：
